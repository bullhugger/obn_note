    ...
    function Scroller(elm, dw, settings) {
        ...
        */
        this.show = function () {
            ...
            $('.dwc', dw).remove();
            $('.calendar-head', dw).remove();
            for (var i = 0; i < s.wheels.length; i++) {
                var dwc = $('<div class="calendar-head"><h5 class="date-title">Handover Date</h5></div><div class="dwc' + (s.mode != 'scroller' ? ' dwpm' : '') + (s.showLabel ? '' : ' dwhl') + '"><div class="dwwc dwrc"><div class="clear" style="clear:both;"></div></div>').insertBefore($('.dwbc', dw));
                // Create wheels
